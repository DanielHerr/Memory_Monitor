...
if(self.document) {
	...
	importScripts("/common/settings.js", "/common/theme.mjs", "changelog.js")
	...
	if(details.reason == "update") {
		...
		if(changelog[version]) {
			chrome.notifications.create("update", {
				type: "list",
				title: "Memory Monitor " + version + " Update",
				iconUrl: "/icon.png",
				message: "",
...
async function render_icon(usage = previous_usage, theme = previous_theme) {
	...
	if(cached) {
		...
		let usage_string = usage.toFixed(1).replace("0", "O")
		let y = canvas.height - canvas.height / 8
		icon.fillText(usage_string[0], canvas.width / 4 - 1, y)
		if(usage < 10) {
			usage_string = usage_string[2]
			icon.fillText(".", canvas.width / 2, canvas.width)
		} else {
			usage_string = usage_string[1]
		}
		icon.fillText(usage_string, canvas.width - (canvas.width / 4) + 1, y)
...
async function refresh_usage() {
	...
	let usage = Number((mem.availableCapacity / units[Settings.measure_unit]).toFixed(1))
	if(usage >= 10) {
	...
	setTimeout(refresh_usage, Settings.numeric_interval * 1000)
}

...
chrome.storage.sync.get({
  clickpanel: true, clickbar: true, clickicon: true, autopanel: false, autoicon: false, low: 0,
  ...
  document.querySelector("#notify").value = items.low;
});
...
chrome.system.memory.getInfo(function(info) {
  document.querySelector("#capacity").textContent = (Math.round(info.capacity / 1024 / 1024 / 100 )) / 10;
});

chrome.permissions.contains({permissions: ["tabs"], origins: ["<all_urls>"] }, function(result) {
...
document.querySelector("#permissions").addEventListener("click", function(){
  ...
  chrome.permissions.request({permissions: ["tabs"], origins: ["<all_urls>"]}, function(granted) {
    if (granted) {}
    else { document.querySelector("#autobar").checked = false }
  }) }
  else { chrome.permissions.remove({permissions: ["tabs"], origins: ["<all_urls>"]}) }
});
...
document.querySelector("form").addEventListener("submit", function(event) {
  ...
  chrome.storage.sync.set({
    ...
    low: document.querySelector("#notify").value
  }, function() {
